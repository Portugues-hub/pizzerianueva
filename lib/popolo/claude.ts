// Informes semanales, alertas de stock y consultas admin vía Claude API (El Rincón de Pepe).
...
export async function generarInformeSemanal(ventas: ResumenVentas): Promise<string> {
  ...
  const prompt = `
Eres el asistente de gestión de Pizzería El Rincón de Pepe
en Callosa de Segura (Alicante).
...
export async function enviarAlertaStock(): Promise<void> {
  ...
  if (!notif?.trim()) {
    console.warn("[Pepe Claude] PEPE_WA_NOTIF no definido; no se envía alerta de stock.");
    return;
  }
  ...
  const mensaje =
    `⚠️ *Alerta de stock — El Rincón de Pepe*\n\n` +
    `${lineas.join("\n")}\n\n` +
...
export async function responderConsultaAdmin(pregunta: string): Promise<string> {
  ...
  const system = `Eres el asistente de gestión de ${MENU.negocio.nombre}.
Tienes acceso al inventario actual: ${stockResumen}.
Responde de forma muy breve (máximo 3 frases) y en español.
...
export async function interpretarMensajeCliente(
  ...
  const system = `Te llamas Braulio y eres el camarero virtual del Rincón de Pepe. Cuando el cliente saluda por primera vez o escribe su primer mensaje, responde SIEMPRE con este texto exacto en el campo respuesta_directa:

👋 ¡Bienvenido! Soy Braulio, tu camarero online del *Rincón de Pepe*.\n\nEstoy aquí para ayudarte con tu pedido. Esto es lo que puedes hacer:\n\n🍕 Escribe *menu* para ver la carta\n🛒 Dime lo que quieres pedir y lo anoto\n💳 Cuando termines, dime *pagar* para cerrar el pedido\n❌ Si quieres cancelar, dime *cancelar*\n\n¡Dime qué te apetece! 😊

Nunca uses la palabra Hola. Usa siempre Bienvenido para saludar.
...
{"intencion":"pedir","items":[{"nombre":"nombre exacto del plato del menú","cantidad":1}]}
...
Si hace una pregunta sobre el menú o el restaurante:
{"intencion":"pregunta","respuesta_directa":"respuesta breve en español"}
