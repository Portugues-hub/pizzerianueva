...
export async function procesarMensaje(from: string, texto: string): Promise<void> {
  ...
  // PASO 4 — LENGUAJE NATURAL CON CLAUDE
  ...
  const menuResumen = formatearMenu();

  let interpretacion;
  try {
    interpretacion = await interpretarMensajeCliente(texto, pedidoResumen, menuResumen);
  } catch {
  ...
  switch (interpretacion.intencion) {
    case "pedir":
      if (interpretacion.items && interpretacion.items.length > 0) {
        const pedido = await obtenerOCrearPedido(from);
        for (const itemSolicitado of interpretacion.items) {
          const item = buscarItem(itemSolicitado.nombre);
          if (item) {
            const existente = pedido.lineas.find((l) => l.item.id === item.id);
            if (existente) existente.cantidad += itemSolicitado.cantidad;
            else pedido.lineas.push({ item, cantidad: itemSolicitado.cantidad });
          }
        }
        await guardarPedido(pedido);
        await enviarMensaje(from, textoResumenPedido(pedido));
      } else {
        await enviarMensaje(from, "No he encontrado ese plato en la carta. Escribe *menu* para ver todos los platos.");
      }
      break;
    case "ver_menu":
      await enviarMensaje(from, formatearMenu());
    ...
    case "ver_pedido": {
      ...
      if (!p || p.lineas.length === 0) {
        await enviarMensaje(from, "No tienes ningún pedido abierto. Escribe *menu* para ver la carta.");
      } else {
    ...
    case "cancelar": {
      ...
      if (!p || p.lineas.length === 0) {
        ...
        await enviarMensaje(from, "Pedido cancelado. Escribe *menu* cuando quieras volver a pedir.");
      }
      break;
    }
    case "pagar": {
      const p = await obtenerPedidoActivoPorFrom(from);
      if (!p || p.lineas.length === 0) {
        await enviarMensaje(from, "No tienes ningún pedido para pagar. Añade platos escribiendo lo que quieres pedir.");
      } else if (p.esperandoMetodoPago) {
        const listoParaMetodo =
          p.tipoEntrega === "local" ||
        ...
        if (listoParaMetodo) {
          ...
          await enviarMensaje(from, "¿Cuál es tu dirección completa para la entrega? 📍");
        }
      } else if (!p.tipoEntrega) {
        p.estado = "pagando";
        await guardarPedido(p);
        await enviarMensaje(from, "¿Cómo quieres recibir tu pedido?\n\n🏪 Escribe *local* para recoger en el restaurante\n🛵 Escribe *domicilio* para recibirlo en casa");
      } else if (p.tipoEntrega === "domicilio" && !p.direccion?.trim()) {
        p.estado = "pagando";
        await guardarPedido(p);
        await enviarMensaje(from, "¿Cuál es tu dirección completa para la entrega? 📍");
      } else {
    ...
    case "otro":
    default:
      await enviarMensaje(from, interpretacion.respuesta_directa ?? "No he entendido tu mensaje. Escribe *menu* para ver la carta.");
      break;
  }
}
